...
type Screen = "day" | "projects" | "jarvis" | "notes";
type Project = { id: string; name: string; status: string; outcome: string; health: string; priority: number; progress: number; lastTouched: string };
...
type JarvisAction = {
  type: "add_task" | "add_priority" | "prioritise_task" | "add_note" | "draft_email" | "calendar_event";
  summary: string;
...
function JarvisView({ projects, today, onSaved }: { projects: Project[]; today: string; onSaved: () => Promise<void> }) {
  ...
  useEffect(() => {
    let active = true;
    fetch(`/api/jarvis?today=${encodeURIComponent(today)}&timezone=${encodeURIComponent(timezone)}`)
      ...
      .catch(() => { if (active) { setError("Jarvis couldn’t load your Command Centre. Try again in a moment."); setLoading(false); } });
    return () => {
      active = false;
  ...
  async function ask(value = input) {
    ...
    try {
      ...
      setAiEnabled(!!data.aiEnabled);
      if (!response.ok && !data.messages) setError(data.error || "Jarvis couldn’t answer that. Your dashboard was not changed.");
      if (data.refreshDashboard) await onSaved();
    } catch {
      setError("Jarvis lost the connection. Your dashboard was not changed — try sending that again.");
    } finally {
      setSending(false);
    }
  }
  ...
  return <div className="grid items-start gap-5 xl:grid-cols-[1.3fr_.7fr]">
    <section className="overflow-hidden rounded-[24px] border border-zinc-200 bg-white shadow-[0_10px_35px_rgba(26,20,54,.08)]">
      ...
      <div ref={threadRef} className="scrollbar-thin min-h-[310px] max-h-[52vh] space-y-4 overflow-y-auto bg-[#f7f7f9] p-4 sm:min-h-[390px] sm:p-5" aria-live="polite">
        ...
        {sending && <div className="flex items-center gap-2 text-sm text-zinc-400"><span className="jarvis-thinking"><i /><i /><i /></span>Jarvis is thinking…</div>}
      </div>
      ...
      <form onSubmit={(event) => { event.preventDefault(); void ask(); }} className="border-t border-zinc-200 bg-white p-3 sm:p-4">
        <div className="relative rounded-2xl border border-zinc-200 bg-white shadow-sm focus-within:border-violet-400 focus-within:ring-4 focus-within:ring-violet-100">
          <Textarea value={input} onChange={(event) => { setInput(event.target.value); setVoiceMessage(null); }} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); void ask(); } }} placeholder="Ask anything, or tell Jarvis what to change…" className="min-h-24 resize-none border-0 bg-transparent px-4 py-3 pr-24 text-base leading-6 shadow-none focus-visible:ring-0" />
          <div className="absolute bottom-2 right-2 flex gap-1.5"><button type="button" onClick={listen} aria-label={voiceActive ? "Stop recording" : "Speak to Jarvis"} className={`grid size-10 place-items-center rounded-xl transition ${voiceActive ? "bg-red-500 text-white" : "bg-zinc-100 text-zinc-600 hover:bg-zinc-200"}`}>{voiceActive ? <Square size={14} fill="currentColor" /> : <Mic size={18} />}</button><button type="submit" disabled={!input.trim() || sending} aria-label="Send to Jarvis" className="grid size-10 place-items-center rounded-xl bg-[#171b21] text-white transition hover:bg-[#7659e8] disabled:cursor-not-allowed disabled:opacity-30"><Send size={17} /></button></div>
        </div>
...
function JarvisActionCard({ messageId, action, status, projects, onReply }: { messageId: number; action: JarvisAction; status: string | null; projects: Project[]; onReply: (message: string) => Promise<void> }) {
  ...
  const label = action.type === "add_task" ? "Add to to-do list" : action.type === "add_priority" || action.type === "prioritise_task" ? "Move to Today" : action.type === "add_note" ? "Save project note" : action.type === "draft_email" ? "Email draft" : "Apple Calendar event";
